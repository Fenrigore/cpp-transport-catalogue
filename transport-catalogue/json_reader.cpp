...
void JsonReader::ProcessRequests(std::ostream& out_str){
	const json::Dict& root_dict = input_doc_.GetRoot().AsMap();
		
	ParseBaseRequests(root_dict.at("base_requests").AsArray());
	ParseRender(root_dict.at("render_settings").AsMap());
...
void JsonReader::ParseStop(const json::Dict& stop_dict) {
	...
	if (stop_dict.count("road_distances")) {
		const json::Dict& road_distances = stop_dict.at("road_distances").AsMap();
...
json::Node JsonReader::ProcessBusRequest(const json::Dict& request)
{
	...
	return json::Dict{
		...
		{"route_length", bus_info.route_distance},
		{"stop_count", static_cast<int>(bus_info.count)},
		{"unique_stop_count", static_cast<int>(bus_info.unique)}
	};
}
